...
public class SendCaseServiceImpl extends BaseController implements SendCaseService{
    ...
    public int save(Map<String, Object> params) {
        ...
        if(content.indexOf("fromAmount")>0) {
            ...
            for (int i = 0; i < ends.length; i++) {
                ...
                Matcher matcher1 = Pattern.compile("^[0-9]\\d*$").matcher(fromAmount);
                Matcher matcher2 = Pattern.compile("^[1-9]\\d*$").matcher(toAmount);
                Matcher matcher3 = Pattern.compile("^[0-9]\\d*%$").matcher(discountAmount);
                Matcher matcher4 = Pattern.compile("^[0-9]\\d*$").matcher(discountAmount);
    ...
    public int update(Map<String, Object> params) {
        ...
        if(content.indexOf("fromAmount")>0) {
            ...
            for (int i = 0; i < ends.length; i++) {
                ...
                Matcher matcher = Pattern.compile("^[0-9]\\d*$").matcher(fromAmount);
    ...
    @Override
    public int removePid(Integer pid) {
 
        //将该方案的指定pid删除
        int result = sendCaseDao.removePid(pid);
        return result;
    }
