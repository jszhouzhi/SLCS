...
public class MerchController extends BaseController {
    ...
    public R save(MerchDo merchant) {
        ...
        if(merchant.getWhiteList()!=null&&!merchant.getWhiteList().equals("")) {
            merchant.setWhiteList(merchant.getWhiteList().replaceAll("\r\n", ","));
        }
    ...
    String edit(Model model, @PathVariable("id") Integer id) {
        ...
        if(merchDo.getWhiteList()!=null&&!merchDo.getWhiteList().equals("")) {
            merchDo.setWhiteList(merchDo.getWhiteList().replaceAll(",", "\n"));
        }
    ...
    @PostMapping("/getCertFile")
    @ResponseBody
    SLResponse getCertFile(String merchantCode) {
    	
		if(merchantCode == null){ 
			return new SLResponse(ErrCode.ARGUMENT_IS_NULL,"获取参数[merchantCode]为NULL值"); 
		}
		SLResponse slResponse = CommUtil.callApiService("api/sl/GetCertFile", merchantCode, true);
		return slResponse; 
